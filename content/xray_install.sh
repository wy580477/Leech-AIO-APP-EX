...
get_current_version() {
    ...
    if [[ -f "${FILES_PATH}/xray" ]]; then
        CURRENT_VERSION="$(${FILES_PATH}/xray -version | awk 'NR==1 {print $2}')"
...
install_xray() {
    install -m 755 ${TMP_DIRECTORY}/xray ${FILES_PATH}/xray
}
...
if [ "${INSTALL_VERSION}" = "${CURRENT_VERSION}" ]; then
    ...
    echo "installed: ${FILES_PATH}/geoip.dat"
    echo "installed: ${FILES_PATH}/geosite.dat"
    "rm" -rf "$TMP_DIRECTORY"
    exit 0
fi
